...
namespace Station {
    namespace Device {
        ...
        QPainter* DeviceBase::m_pPainter = new QPainter;
        ...
        DeviceBase::DeviceBase(QObject* parent)
        {
            ...
            m_mapAttribute.insert("pName", [&](const QString& strElement) { m_ptName = QStringToQPoint(strElement); });
            m_mapAttribute.insert("m_nCode", [&](const QString& strElement) { m_nCode = strElement.toUInt(nullptr, 16); });
            m_mapAttribute.insert("m_textRect", [&](const QString& strElement) { m_rcTextRect = QStringToQRect(strElement); });
        ...
        void DeviceBase::DrawDeviceName(const bool& bElapsed)
        {
            ...
            if (m_rcTextRect != QRect()) {
                m_pPainter->drawText(Scale(QRect(m_rcTextRect.topLeft(), fontMetrics.size(Qt::TextSingleLine, m_strName))), m_strName, QTextOption(Qt::AlignCenter));
            }
        ...
        {
            return (value + nOffset) * m_pStationObject->m_nDiploid;
        }

        QPoint DeviceBase::Scale(const QPoint& pt, const bool bOutSide, const bool bTopLine)
        {
            if (!bOutSide) {
                return QPoint(Scale(pt.x(), m_pStationObject->m_ptOffset.x()), Scale(pt.y(), m_pStationObject->m_ptOffset.y()));
            }
            else {
                if (bTopLine) {
                    return QPoint(Scale(pt.x(), m_pStationObject->m_ptOffset.x()), Scale(pt.y() - TRACK_WIDTH / 2, m_pStationObject->m_ptOffset.y()) + 1);
                }
                else {
                    return QPoint(Scale(pt.x(), m_pStationObject->m_ptOffset.x()), Scale(pt.y() + TRACK_WIDTH / 2, m_pStationObject->m_ptOffset.y()));
                } 
            }
        }
        ...
        {
            return QRect(Scale(rect.x(), m_pStationObject->m_ptOffset.x()), 
                    Scale(rect.y(), m_pStationObject->m_ptOffset.y()),
